 ...
 * Scheduling tools.
 *
 * Maps to upstream `container/agent-runner/src/mcp-tools/scheduling.ts` slot.
 * Tools: schedule_task, list_tasks, pause_task, resume_task, cancel_task, update_task.
 *
 * GHC variant preserves cron-parser validation + IPC file writes.
 */
import fs from 'fs';
...
server.tool(
  ...
  async (args) => {
    ...
    return {
      content: [{ type: 'text' as const, text: `Task ${args.task_id} pause requested.` }],
    };
  },
);
...
server.tool(
  ...
  async (args) => {
    ...
    return {
      content: [{ type: 'text' as const, text: `Task ${args.task_id} resume requested.` }],
    };
  },
);
