/**
 * Memory loader (MIRRORED from host package src/memory/loader.ts).
 *
 * Container runners are separate npm packages that don't import host
 * source. Keep this file in sync if you change the host loader. The
 * host's vitest suite exercises the canonical version; this copy is
 * thin enough that drift is easy to spot in code review.
 *
...
export function loadMemoryFromEnv(): LoadedMemory {
  ...
  // groupFolder is the parent of memoryDir for the loader's API.
