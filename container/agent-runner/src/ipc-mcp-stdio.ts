...
// ---------------------------------------------------------------------------
// Agent-facing API for nanoclaw's per-group memory store. All paths are
// resolved from $NANOCLAW_MEMORY_DIR (set by host) or fall back to
// <groupFolder>/memory. Date/time formatting uses $NANOCLAW_TZ (set by
// host from nanoclaw.json `timezone`) so daily filenames are local-time.
//
// If you change this block, mirror it in the sibling runner
// (container/agent-runner{,-ghc}/src/ipc-mcp-stdio.ts).
...
server.tool(
  ...
  async (args) => {
    ...
    if (!fs.existsSync(dir)) {
      return {
        content: [{ type: 'text' as const, text: `No matches for "${args.query}".` }],
      };
    }
...
server.tool(
  'memory_append_today',
  "Append a note to TODAY's daily journal (YYYY-MM-DD.md, local time). Use for capturing events, decisions, or anything worth recalling tomorrow. NEVER store secrets, tokens, or PII. Each call adds a bullet with a local-time HH:MM prefix.",
  {
