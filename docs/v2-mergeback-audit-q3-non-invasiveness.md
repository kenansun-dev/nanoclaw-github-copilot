...
| `src/config.ts` | **(reclassified 2026-04-30 P1.3 audit; deferred to B.x cutover — rpi5+VM agreed 2026-04-30, owner delegated)** Data-source model entirely replaced: upstream is `process.env || envConfig.X || 'default'`; fork is `_config.agents.defaults.X` (loads from `nanoclaw.json` via `config-loader.ts`). Same export names, different signatures + side-effects. Was mis-bucketed as ADDITIVE in original audit because diff size (+87/-44) didn't reflect semantic replacement. | **Deferred to B.x cutover** alongside `index.ts` / `container-runner.ts` / `task-scheduler.ts`. See `docs/v2-merge-triage-2026-04-30.md` § "`src/config.ts` — restore upstream + add `config-extensions.ts` overlay" for cutover plan. |
