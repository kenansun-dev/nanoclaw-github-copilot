...
import { isFinalOutput, makeCloseSentinelState, shouldWriteCloseSentinel } from './tui-direct-close-sentinel.js';

describe('isFinalOutput', () => {
  it('partial chunks are not final', () => {
    expect(isFinalOutput({ partial: true, status: 'success', result: 'x' })).toBe(false);
  });
  ...
  it('"thinking" status with no result is not final (still-working pulse)', () => {
    ...
    expect(isFinalOutput({ status: 'thinking', result: undefined })).toBe(false);
  });
  ...
  it('custom status with non-empty result is final', () => {
    expect(isFinalOutput({ status: 'partial-stream-done', result: 'P' })).toBe(true);
  });
...
describe('shouldWriteCloseSentinel — once-only gate', () => {
  it('returns true exactly once for the first final output', () => {
    ...
    expect(shouldWriteCloseSentinel(s, { status: 'success', result: 'PONG' })).toBe(true);
    // Second final output: must NOT trigger another close
    expect(shouldWriteCloseSentinel(s, { status: 'success', result: 'PONG2' })).toBe(false);
  });
  ...
  it('returns false for partial / thinking outputs preceding the final', () => {
    ...
    expect(shouldWriteCloseSentinel(s, { partial: true, status: 'success' })).toBe(false);
    expect(shouldWriteCloseSentinel(s, { status: 'thinking', result: null })).toBe(false);
    // …and then the first FINAL fires the close
    expect(shouldWriteCloseSentinel(s, { status: 'success', result: 'PONG' })).toBe(true);
    // …and a subsequent final still does not
    expect(shouldWriteCloseSentinel(s, { status: 'success', result: 'X' })).toBe(false);
  });
  ...
  it('independent state objects have independent close-once semantics', () => {
    ...
    expect(shouldWriteCloseSentinel(s1, { status: 'success', result: 'a' })).toBe(true);
    // s2 has not seen anything yet — must still fire once
    expect(shouldWriteCloseSentinel(s2, { status: 'success', result: 'b' })).toBe(true);
    // But s1 is now consumed
    expect(shouldWriteCloseSentinel(s1, { status: 'success', result: 'c' })).toBe(false);
  });
