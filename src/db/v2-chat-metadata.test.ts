...
import { getRegisteredGroupV2, getAllRegisteredGroupsV2, compareV1V2ChatMetadata } from './v2-chat-metadata.js';
...
describe('v2-chat-metadata bridge', () => {
    ...
    seedV2([
      {
        id: 'mg-2',
        channel_type: 'teams',
        platform_id: 'a:1Rw3-thread-id',
        name: 'team',
        agent_group_id: 'teams-main',
      },
    ]);
  ...
  it('getAllRegisteredGroupsV2 skips unwired MG rows (no MGA join)', () => {
    ...
    // unwired row
    getDb()
      .prepare(`INSERT INTO messaging_groups (id, channel_type, platform_id, created_at) VALUES (?,?,?,?)`)
      .run('mg-y', 'telegram', '88', '2026-05-16T00:00:00Z');
    const all = getAllRegisteredGroupsV2();
  ...
  describe('compareV1V2ChatMetadata', () => {
    ...
    it('reports zero drift when matching', () => {
      const v1: Record<string, RegisteredGroup> = {
        'tg:1': { name: 'kenan', folder: 'main', trigger: '', added_at: 't' },
      };
      const v2 = { 'tg:1': v2Row };
    ...
    it('reports field mismatch (folder)', () => {
      const v1: Record<string, RegisteredGroup> = {
        'tg:1': { name: 'kenan', folder: 'OTHER', trigger: '', added_at: 't' },
      };
