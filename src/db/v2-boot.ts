 ...
 *
 * Opens the v2 central DB (same on-disk file as the legacy
 * `initDatabase()` — `<workspace>/store/messages.db`), runs migrations
 * (the v2-boot-guard inside `runMigrations` defuses the legacy
 * `sessions` table first), and reconciles declared config into v2
 * tables.
 *
...
export function initAndReconcileV2(): V2BootResult {
  const dbPath = workspacePath('store', 'messages.db');
