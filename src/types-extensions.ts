/**
 * Fork extensions to upstream `src/types.ts`.
 *
 * Keeps `src/types.ts` upstream-verbatim for cheap merges. All fork-only
 * type definitions live here:
 *   - Container/sandbox security: `AdditionalMount`, `MountAllowlist`,
 *     `AllowedRoot`, `ContainerConfig`
 *   - Registered groups (v1 dispatcher surface): `RegisteredGroup`
 *   - v1 messaging primitives: `NewMessage`, `ScheduledTask`, `TaskRunLog`,
 *     `StreamHandle`, `Channel`, `OnInboundMessage`, `OnChatMetadata`
 *
 * History: prior to this extraction these lived inline at the top of
 * `src/types.ts` as a "230-line fork prepend block". Owner asked for
 * extension-pattern discipline (Q2 followup audit, Discord 2026-04-30).
 *
 * Re-export upstream types so callers can import a single module if
 * convenient.
 */
